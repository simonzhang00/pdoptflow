...
namespace lemon {
    ...
    public:
        ...
        void first(Node& n) const {
            //n.id = 0;
            n.id= node_num-1;
            //std::cout<<"first nodeit: "<<n.id<<std::endl;//added by Simon Zhang
        }
        static void next(Node& n) {
            //++n.id;
            --n.id;
            //std::cout<<"next nodeit: "<<n.id<<std::endl;//added by Simon Zhang
        }
        ...
        static void next(Arc& e) {
            //++e.id;
            --e.id;
            //std::cout<<"next arcid: "<<e.id<<std::endl;//added by Simon Zhang
        }
        /*
        void first(Node& n) const {
            n.id = node_num - 1;
...
*/
    ...
    public:
        ...
        void build_parallel(int n, ArcList arcs,  std::vector<int> offsets) {
            built = true;

            node_num = n;
            arc_num = std::distance(arcs.begin(), arcs.end());
...
#pragma omp parallel for schedule(static,1)
            for (int i = 0; i < node_num; ++i) {
                //std::cout<<i<<std::endl;
                int arc_index = offsets[i];
                node_first_out[i] = arc_index;
                for (; arc_index < offsets[i + 1]; arc_index++) {
                    int j = arcs[arc_index].second;
                    LEMON_ASSERT(j >= 0 && j < node_num,
                                 "Wrong arc list for StaticDigraph::build_parallel()");
            ...
            //this assert is useless since we work in parallel
//                LEMON_ASSERT(arc_index == arcs.size(),
//                             "Wrong arc list for StaticDigraph::build()");
        ...
        void build(int n, ArcListIterator first, ArcListIterator last) {
            ...
            for (int i = 0; i != node_num; ++i) {
                ...
                //Simon Zhang: you can parallelize the outer loop and work in prefix sum offsets in inner loop...
