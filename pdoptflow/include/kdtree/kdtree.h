/**
modified code from https://github.com/gishi523/kd-tree
    ...
    private:
        ...
        */
        Node* buildRecursive(int* indices, int npoints, int depth)
        {
            ...
            const int axis = depth % 2;//PointT::DIM;
        ...
        */
        ...
        static double distance( PointT p, PointT q)
        {
            return p.dist_l2(q);
            //return p.dist_linfty(q);
            //double dist = 0;
        ...
        */
        void nnSearchRecursive(const PointT& query, const Node* node, int *guess, double *minDist) const
        {
            ...
            int dir= query.x < train.x ? 0: 1;
            if(axis%2==1){
                dir= query.y< train.y ? 0:1;
            }
            nnSearchRecursive(query, node->next[dir], guess, minDist);

//            const double diff = fabs(query[axis] - train[axis]);
            double diff = fabs(query.x - train.x);
            if(axis%2==1){
                diff= fabs(query.y -train.y);
            }
            if (diff < *minDist)
                nnSearchRecursive(query, node->next[!dir], guess, minDist);
        }
        ...
        */
        void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, double radius) const
        {
            ...
            //Anonymous: this can be replaced by l2 distance, but linfty might be faster?
            const double diff = fabs(query[axis] - train[axis]);
            if (diff < radius)
                radiusSearchRecursive(query, node->next[!dir], indices, radius);
        }
