...
namespace pde {
    ...
    public:
        ...
        void load_points(NumPyFloatArray points) {
            ...
            std::cout<<"max depth of quadtree: "<<depth_of_quadtree<<"\n";
            num_queries = 0;
    ...
    public:
        ...
        void build_quadtree_pd(const std::vector<int32_t> &subset,
            ...
            depth_of_quadtree= std::max(depth+1, depth_of_quadtree);
            ...
            if(subset.size()==2){
//std::cout<<std::setprecision(16)<<"subset[0], subset[1]"<<(*dictionary)(subset[0],0)<<"," <<(*dictionary)(subset[0],1)<<" "<<(*dictionary)(subset[1],0)<<","<<(*dictionary)(subset[1],1)<<std::endl;
            }
            if(subset.size()>1){
                bool allsame= true;
                float eps= 1e-15;
                for(int i=1; i<subset.size(); i++){
                    if((*dictionary)(subset[i],0)!=(*dictionary)(subset[0],0) || (*dictionary)(subset[i],1)!=(*dictionary)(subset[0],1)){
                        allsame= false;
                        break;
                    }
                }
