...
#   - 2020/11 Simon Zhang: did not change the file except put in some profiling code;
##TO USE: python3 wasserstein_emd2_POT_GUDHI.py file1 file2 1
##this computes the 1-wasserstein distance between PD1 and PD2 in PD format
...
def wasserstein_distance(X, Y, matching=False, order=1., internal_p=np.inf, enable_autodiff=False):
    ...
    M= M+M.T
    print(np.allclose(M,M.T, rtol=rtol, atol=atol))

    a= np.random.random(len(M))
    a= a/np.sum(a)
    b= np.random.random(len(M))
    b= b/np.sum(b)
    tic = time.time()
    ot_cost = emd(a, b, M)
    print(ot_cost)
    toc = time.time()
    print("emd time: ", toc - tic, "s")
    '''
    ...
    ##F= ot.gpu.sinkhorn(a,b,M,0.5) 
    ##ot_cost= np.dot(F.ravel(),M.ravel())
    toc = time.time()
    print("emd2 time: ", toc - tic, "s")
...
if __name__ == "__main__":
    ...
    print(wasserstein_distance(X, Y, False, int(sys.argv[3]), internal_p=2))
    toc = time.time()
