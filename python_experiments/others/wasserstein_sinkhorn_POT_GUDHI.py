...
#   - 2020/11 Simon Zhang: modified to run with sinkhorn from POT instead of emd2

##TO USE: python3 wasserstein_sinkhorn_POT_GUDHI.py file1 file2 1 regularization-param
##this computes the 1-wasserstein distance between PD1 and PD2 in PD format where regularization-param is the regularization parameter to sinkhorn
##regularization parameter too small will result in numerical errors.
import numpy as np
...
def wasserstein_distance(X, Y, matching=False, order=1., internal_p=np.inf, enable_autodiff=False):
    ...
    if enable_autodiff:
        ...
        if len(pairs_X_Y):
            dists.append(
                (Y_orig[pairs_X_Y[:, 1]] - X_orig[pairs_X_Y[:, 0]]).norms.lp(internal_p, axis=-1).norms.lp(order))
    ...
    # F= ot.sinkhorn(a,b,M, 0.0005)
    tic = time.time()
    F = ot.gpu.sinkhorn(a, b, M, float(sys.argv[3]))
    ot_cost = np.dot(F.ravel(), M.ravel())
    toc = time.time()
    print("sinkhorn computation time (no memory transfer time): ", toc - tic, "s")
...
if __name__ == "__main__":
    ...
    print(wasserstein_distance(X, Y, False, 1, internal_p= 2))
    toc = time.time()
    print("W1 time: ", toc - tic)
