...
def find_candidates(method, n_resulting_candidates, query_diagram_index, candidate_diagram_indices):
    ...
    for candidate in candidate_diagram_indices:
        ...
        if method=="wcd":
            ...
            w1= W1.wasserstein1(query_diagrams[query_diagram_index], point_diagrams[p_index], 18, 10000)
            result.append([w1, p_index])
...
if __name__ == "__main__":
    ...
    n_wcd= 95
    n_rwmd= 10
    n_ft= 15#FOR NUMBER OF FILTERED REMAINING AFTER FT
    n_qt= 95
    n_sparse= 3#FOR NUMBER OF FILTERED REMAINING AFTER PDoptFlow(s=1)
    n_exact= topK#FOR NUMBER OF FILTERED REMAINING AFTER PDoptFlow(s=18)
    n_correct= 0
    n_correct_qt= 0
    n_correct_ft= 0
    n_correct_rwmd= 0
    ...
    for i,q in enumerate(tqdm(query_diagrams)):
        ...
        start= time.time()
        sparse_candidates= find_candidates("sparse", n_sparse, i, flowtree_candidates)
        stop= time.time()
        sp_time+= (stop-start)
        start= time.time()
        
        final_candidates= find_candidates("exact", n_exact, i, sparse_candidates)#flowtree_candidates)
        stop= time.time()
        ...
        print("final_candidates: [approx, NN index]", final_candidates[0:topK])
    ...
    print("PDoptFlow(s=1) SPARSE CORRECT 1-NNs: ", n_correct_sp)
    ...
    print("PDoptFlow(s=1) sparse time: ", sp_time)
